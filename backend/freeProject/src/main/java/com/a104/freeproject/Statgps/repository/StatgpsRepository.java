...
public interface StatgpsRepository extends JpaRepository<Statgps,Long> {
    @Query(value="select chk, success from statgps where prt_chl_id=:p and date_format(chk, '%Y-%m-%d')=:date group by chk, success order by chk", nativeQuery = true)
    List<GpsMiddleInterface> findByChkAndPrtChl(@Param("date") String date, @Param("p") PrtChl p);
